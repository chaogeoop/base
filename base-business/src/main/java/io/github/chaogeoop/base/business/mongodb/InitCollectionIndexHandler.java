...
public class InitCollectionIndexHandler {
    ...
    private void setProperties(ExecutorService executorService) {
        ...
        this.init(executorService, mongoInitUnitList, unit -> EnhanceBaseModelManager.getAccordCollectionNamePreInit(unit.getMongoTemplate(), unit.getClazz(), unit.getCollectionName()));
        ...
        for (Class<? extends ISearch<? extends IBaseEs>> clazz : this.searchClazzList) {
            ...
            for (MongoTemplate database : databaseList) {
                ...
                EnhanceBaseModelManager.DatabaseUnit databaseUnit = EnhanceBaseModelManager.DatabaseUnit.of(database, baseCollectionName);
